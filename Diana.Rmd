...
title: "HomeWork 4"
author: 'Gainutdinova Diana'
date: "deadline - 01.06.2015"
output:
  pdf_document:
    fig_caption: yes
    toc: yes
...
Send ONLY hw4_Surname.Rmd versions of your report.
...
$$F(+\infty) = 1 = \int_{x_{min}}^{\inf} p(x)dx=C\int_{x_{min}}^{\inf} x^{-a}dx = \frac{C}{1-a} [x^{1-a}]_{x_{min}}^{\inf}$$

$$C = (a-1)x_{min}^{a-1}$$, and 

$$F(x) = C \left( -\frac{x^{1 - \alpha}}{\alpha-1} + \frac{x_{min}^{1 - \alpha}}{\alpha-1} \right) = (a-1)x_{min}^{a-1} \left( -\frac{x^{1 - \alpha}}{\alpha-1} + \frac{x_{min}^{1 - \alpha}}{\alpha-1} \right) = -x_{min}^{a-1} x^{1-a}+1=1-(\frac{x}{x_{min}})^{1-a}$$
...
# fit linear regression
c_reg <- lm(log(cum_degree_dist) ~ log(cum_degree_range))
c_coefficients <- coef(c_reg)
c_func_powerlaw <- function(x) exp(c_coefficients[[1]] + c_coefficients[[2]] * log(x))
c_alpha <- -c_coefficients[[2]]
...
curve(c_func_powerlaw, col = "green", add = TRUE, n = length(cum_degree_range))
...
```{r}
d <- degree(graph)
fit = power.law.fit(d, implementation = "plfit")
alpha = fit$alpha
xmin = max(fit$xmin)
...
curve(func_powerlaw, col = "green", add = TRUE, n = length(degree_range))
```
...
k1=sum(is.loop(graph, eids=E(graph)))
k2=sum(is.multiple(graph, eids=E(graph)))
k3=sum(count.multiple(graph, eids=E(graph)))
print(k1)
print(k2)
print(k3)
